...
class BaseStrategyFrame(ttk.Frame):
    ...
    def initialize_config(self, loadxbridgeconf: bool = True):
        try:
            ...
            if self.config_manager:
                self.config_manager.error_handler.handle(e, context={"strategy": self.strategy_name})
    ...
    def _critical_error_handler(self, error):
        ...
        self.main_app.status_var.set(error_msg)
        if self.config_manager:
            self.config_manager.error_handler.handle(error, context={"strategy": self.strategy_name})
    ...
    def _finalize_stop(self, reload_config: bool = True):
        if self.config_manager:
            self.config_manager.general_log.debug("GUI: Finalizing stop for %s. Reload config: %s", self.strategy_name, reload_config)
    ...
    def cancel_all(self):
        ...
        async def worker_task():
            try:
                await self.config_manager.xbridge_manager.cancelallorders(use_shutdown_event=False)
                ...
                self.config_manager.error_handler.handle(e, context={"stage": "cancel_all"})
            finally:
    ...
    def _fetch_orders_data(self) -> List[Dict[str, Any]]:
        ...
        with self.config_manager.resource_lock:
            for pair_obj in self.config_manager.pairs.values():
                ...
                orders.append({
                    "name": name, "symbol": symbol, "status": status, "side": current_order_side,
                    "flag": self._get_flag(status), "variation": variation_display, "maker_size": maker_size,
                    "maker": maker, "taker_size": taker_size, "taker": taker, "dex_price": dex_price,
                    "order_id": order_id
                })
        return orders
    ...
    def on_closing(self):
        if self.config_manager:
            self.config_manager.general_log.info("Closing %s strategy...", self.strategy_name)
        self.stop(reload_config=False)

    def reload_configuration(self, loadxbridgeconf: bool = True):
        if not self.config_manager:
            return
        self.config_manager.general_log.debug("GUI: Reloading configuration for %s.", self.strategy_name)
    ...
    def create_standard_buttons(self):
        ...
        self.btn_stop = ttk.Button(button_frame, text="STOP", command=lambda: self.stop(), width=btn_width)
