...
class TreeManager:
    ...
    def __init__(
        ...
        self.tree = ttk.Treeview(
            self.parent, columns=[c[0] for c in columns], show="headings"
        )
    ...
    def _setup_tree(self):
        ...
        def set_column_weights(event=None):
            ...
            for col_id, _, weight in self.columns:
                self.tree.column(
                    col_id, width=int(width * (weight / total_weight)), stretch=True
                )
        ...
        for col_id, col_name, _ in self.columns:
            self.tree.heading(
                col_id, text=col_name, command=lambda c=col_id: self._sort_column(c)
            )
    ...
    def _sort_data(self, items: list[dict]) -> list[dict]:
        ...
        return sorted(
            items,
            key=lambda k: self._get_sort_value(k.get(self.sort_column, "")),
