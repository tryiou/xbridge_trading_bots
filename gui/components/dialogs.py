...
class Validators:
    ...
    @staticmethod
    def numeric_range(value: str, min_val: float, max_val: float | None = None) -> bool:
    ...
    def _validate_pair(self, pair_var):
        ...
        if not Validators.trading_pair(pair):
            self.config.update_status(
                "Invalid pair format. Must be like BTC/USDT (3-5 uppercase letters)",
                "red",
            )
            return False
        return True
...
class BasePairDialog(BaseDialog):
    ...
    def __init__(
        ...
        self.enabled_var = tk.BooleanVar(
            value=True if values is None else values[1] == "Yes"
        )
        ...
        self.sell_offset_var = tk.StringVar(
            value="0.05" if values is None else values[4]
        )
    ...
    def _create_widgets(self):
        ...
        ttk.Label(self, text="Price Variation Tolerance:").grid(
            row=3, column=0, padx=5, pady=2, sticky="w"
        )
        ttk.Entry(self, textvariable=self.var_tol_var).grid(
            row=3, column=1, padx=5, pady=2
        )
        ttk.Label(self, text="Sell Price Offset:").grid(
            row=4, column=0, padx=5, pady=2, sticky="w"
        )
        ttk.Entry(self, textvariable=self.sell_offset_var).grid(
            row=4, column=1, padx=5, pady=2
        )
        ttk.Label(self, text="USD Amount:").grid(
            row=5, column=0, padx=5, pady=2, sticky="w"
        )
        ttk.Entry(self, textvariable=self.usd_amt_var).grid(
            row=5, column=1, padx=5, pady=2
        )
        ttk.Label(self, text="Spread:").grid(
            row=6, column=0, padx=5, pady=2, sticky="w"
        )
        ttk.Entry(self, textvariable=self.spread_var).grid(
    ...
    def _create_buttons(self, ok_text: str, ok_command: callable):
        ...
        ttk.Button(btn_frame, text=ok_text, command=ok_command).pack(
            side="left", padx=5
        )
        ttk.Button(btn_frame, text="Cancel", command=self.destroy).pack(
            side="left", padx=5
        )
        self.bind("<Return>", lambda _: ok_command())
...
class BaseSellerDialog(BaseDialog):
    ...
    def _create_widgets(self):
        ...
        self.name_entry = ttk.Entry(self, textvariable=self.name_var)
        self.name_entry.grid(row=1, column=1, padx=5, pady=2)
        ttk.Label(self, text="Pair (MAKER/TAKER):").grid(
            row=2, column=0, padx=5, pady=2, sticky="w"
        )
        self.pair_entry = ttk.Entry(self, textvariable=self.pair_var)
        self.pair_entry.grid(row=2, column=1, padx=5, pady=2)
        ttk.Label(self, text="Amount to Sell:").grid(
            row=3, column=0, padx=5, pady=2, sticky="w"
        )
        ttk.Entry(self, textvariable=self.amount_var).grid(
            row=3, column=1, padx=5, pady=2
        )
        ttk.Label(self, text="Min Sell Price (USD):").grid(
            row=4, column=0, padx=5, pady=2, sticky="w"
        )
        ttk.Entry(self, textvariable=self.min_price_var).grid(
            row=4, column=1, padx=5, pady=2
        )
        ttk.Label(self, text="Sell Price Offset:").grid(
            row=5, column=0, padx=5, pady=2, sticky="w"
        )
        ttk.Entry(self, textvariable=self.offset_var).grid(
            row=5, column=1, padx=5, pady=2
        )
