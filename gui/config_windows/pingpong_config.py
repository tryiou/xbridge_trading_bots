...
class GUI_Config_PingPong(BaseConfigWindow, TreeviewMixin):
    ...
    def _create_widgets(self, parent_frame: ttk.Frame):
        _canvas, content_frame = self._create_scrollable_content_frame(parent_frame)
    ...
    def _create_general_settings_widgets(self, parent_frame: ttk.Frame) -> None:
        ...
        ttk.Label(general_frame, text="Debug Level:").grid(
    ...
    def _create_pairs_treeview_widgets(self, parent_frame: ttk.Frame) -> None:
        ...
        columns = [
            ...
            ("enabled", "Enabled"),
            ("pair", "Pair"),
            ("price_variation_tolerance", "Var. Tol."),
            ("sell_price_offset", "Sell Offset"),
            ("usd_amount", "USD Amt"),
            ("spread", "Spread"),
        ]
        self.pairs_treeview = self._create_treeview_with_scrollbar(
            tree_frame, columns, height=8
        )
        ...
        col_configs = {
            ...
            "enabled": (75, "center"),
            "pair": (150, "w"),
            "price_variation_tolerance": (120, "e"),
    ...
    def edit_pair_config(self) -> None:
        ...
        if selected:
            ...
            if dialog.result:
                ...
                self.update_status(
                    f"Pair {dialog.result[2]} updated successfully.", "lightgreen"
                )
            else:
        ...
        if self.pairs_treeview:
            for item_id in self.pairs_treeview.get_children():
                ...
                if not values[0].strip():
                    self.update_status(
                        "Name is required for each pair configuration", "red"
                    )
                    return None
                ...
                if "/" not in pair or len(pair.split("/")) != 2:
                    self.update_status(
                        f"Invalid pair format: {pair}. Must be TOKEN1/TOKEN2", "red"
                    )
                    return None
                ...
                try:
                    ...
                    self.update_status(
                        f"Invalid numeric value in pair config: {e}", "red"
                    )
                    if self.parent.config_manager:
                        self.parent.config_manager.general_log.error(
                            f"Failed to parse pair config: {e}"
                        )
                    return None
