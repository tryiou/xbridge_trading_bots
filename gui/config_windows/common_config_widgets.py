...
class TreeviewMixin:
    ...
    def _setup_scroll_bindings(self, canvas: tk.Canvas) -> None:
        ...
        if self.config_window:
            self.config_window.bind(
                "<Up>", lambda event: self._on_key_press_scroll(event, canvas, -1)
            )
            self.config_window.bind(
                "<Down>", lambda _: self._on_key_press_scroll(_, canvas, 1)
            )
            self.config_window.bind(
                "<Prior>", lambda _: canvas.yview_scroll(-10, "units")
            )
            self.config_window.bind(
                "<Next>", lambda _: canvas.yview_scroll(10, "units")
            )
            self.config_window.bind(
                "<MouseWheel>",
    ...
    def _create_treeview_with_scrollbar(
        ...
        scrollbar = ttk.Scrollbar(
            parent_frame, orient="vertical", command=treeview.yview
        )
    ...
    def _create_control_buttons_for_treeview(
        ...
        ttk.Button(btn_frame, text=add_text, command=add_command).pack(
            side="left", padx=2
        )
        ttk.Button(btn_frame, text=remove_text, command=remove_command).pack(
            side="left", padx=2
        )
        ttk.Button(btn_frame, text=edit_text, command=edit_command).pack(
            side="left", padx=2
        )
