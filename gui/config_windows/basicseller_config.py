...
class GUI_Config_BasicSeller(BaseConfigWindow, TreeviewMixin):
    ...
    def _create_sellers_treeview(self, parent_frame: ttk.Frame) -> None:
        ...
        columns = [
            ...
            ("min_sell_price_usd", "Min Price (USD)"),
            ("sell_price_offset", "Offset"),
        ]
        self.sellers_treeview = self._create_treeview_with_scrollbar(
            tree_frame, columns, height=10
        )
        ...
        col_configs = {
            ...
            "enabled": (75, "center"),
            "pair": (150, "w"),
            "amount_to_sell": (120, "e"),
            "min_sell_price_usd": (150, "e"),
        ...
        if self.sellers_treeview:
            ...
            if selected:
                ...
                if dialog.result:
                    ...
                    self.update_status(
                        f"Seller {dialog.result[2]} updated successfully.", "lightgreen"
                    )
                else:
        ...
        if self.sellers_treeview:
            for item_id in self.sellers_treeview.get_children():
                ...
                try:
                    ...
                    self.update_status(
                        f"Invalid numeric value in seller config: {e}", "red"
                    )
                    if self.parent.config_manager:
                        self.parent.config_manager.general_log.error(
                            f"Failed to parse seller config: {e}"
                        )
                    return None
