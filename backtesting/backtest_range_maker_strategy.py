...
class TradeRecord:
    ...
    price: float
    base_price: float  # Price before spread was applied
    base_amount: float
    quote_amount: float
    order_id: str = ""
...
class DataManager:
    ...
    def _load_csv_data(self, file_path: Path) -> pd.DataFrame:
        ...
        try:
            self.logger.debug(f"Loading CSV from: {file_path}")
            data = pd.read_csv(file_path, parse_dates=['date'])
            self.logger.debug(f"Raw data columns: {data.columns.tolist()}")
            self.logger.debug(f"Raw data shape: {data.shape}")
            ...
            # Convert OHLC columns to float (handle Forex data)
            for col in ['open', 'high', 'low', 'close']:
                if col in data.columns:
                    data[col] = pd.to_numeric(data[col], errors='coerce')

            # Log raw data issues
            self.logger.debug(f"Before processing - Index duplicates: {data.duplicated(subset=['timestamp']).sum()}")
            self.logger.debug(f"Before processing - Null timestamps: {data['timestamp'].isnull().sum()}")

            # Set index and ensure sorted with no duplicates
            data = data.set_index('timestamp')
            self.logger.debug(f"After set_index - Index type: {type(data.index)}")
            self.logger.debug(f"After set_index - Index duplicates: {data.index.duplicated().sum()}")
            self.logger.debug(f"After set_index - Index monotonic: {data.index.is_monotonic_increasing}")
            ...
            self.logger.debug(f"After dedupe/sort - Index duplicates: {data.index.duplicated().sum()}")
            self.logger.debug(f"After dedupe/sort - Index monotonic: {data.index.is_monotonic_increasing}")
            self.logger.debug(f"Index range: {data.index.min()} to {data.index.max()}")

            if not data.index.is_monotonic_increasing:
                self.logger.error("Index is NOT monotonic after processing!")
                self.logger.debug(f"Problematic index values: {data.index}")
                raise ValueError("Index not monotonic after processing")

            # Resample and clean
            self.logger.debug("Attempting resampling...")
            data = data.resample('1D').bfill()
            ...
            self.logger.debug(f"Data head: {data.head() if 'data' in locals() else 'No data'}")
            self.logger.debug(f"Data tail: {data.tail() if 'data' in locals() else 'No data'}")
            raise

    async def _download_forex_data(self, pair: str, config: BacktestConfig) -> pd.DataFrame:
        """Download and process Forex data for EUR/USD or USD/EUR."""
        import yfinance as yf

        # Use the standard EUR/USD symbol
        symbol = "EURUSD=X"
        self.logger.info(f"Downloading Forex pair: {symbol}")
        data = yf.download(symbol, period=config.period, interval=config.timeframe, progress=False, auto_adjust=True)
        ...
        # Invert prices for USD/EUR
        if pair == "USD/EUR":
            # Store original values before inversion
            ...
            # Invert prices
            df['open'] = 1 / original_open
            df['high'] = 1 / original_low  # High becomes 1/original_low
            df['low'] = 1 / original_high  # Low becomes 1/original_high
            df['close'] = 1 / original_close

        return df

    async def _download_crypto_data(self, pair: str, config: BacktestConfig) -> pd.DataFrame:
        """Download and process crypto pair data."""
        import yfinance as yf
        ...
        base_data = yf.download(base_sym, period=config.period, interval=config.timeframe, progress=False,
                                auto_adjust=True)
        quote_data = yf.download(quote_sym, period=config.period, interval=config.timeframe, progress=False,
                                 auto_adjust=True)

        return self._create_synthetic_pair_data(base_data, quote_data)

    async def _download_data(self, pair: str, file_path: Path, config: BacktestConfig):
        """Download historical data using appropriate method."""
        try:
            import yfinance as yf
        except ImportError:
        ...
        try:
            # Handle specific Forex pairs
            if pair in ["EUR/USD", "USD/EUR"]:
                df = await self._download_forex_data(pair, config)
            else:
                df = await self._download_crypto_data(pair, config)

            # Set volume to 0 and save
            df['volume'] = 0
...
class PortfolioTracker:
    ...
    def execute_trade(self, order: Dict[str, Any], timestamp: datetime, base_price: float = None) -> Optional[
        TradeRecord]:
        ...
        # Use provided base_price or fall back to order price
        trade_base_price = base_price if base_price is not None else order['price']
...
class ReportGenerator:
    """Generates formatted performance reports with dual-sided metrics."""
    ...
    def generate_report(self) -> str:
        """Generate comprehensive performance report with dual-sided metrics."""
        ...
        # Calculate IL in base token
        final_price = self.metrics.final_price
        final_il_base = (self.metrics.final_impermanent_loss / final_price
                         if final_price != 0 else 0)
        max_il_base = (self.metrics.max_impermanent_loss / final_price
                       if final_price != 0 else 0)
        avg_il_base = (self.metrics.avg_impermanent_loss / final_price
                       if final_price != 0 else 0)

        # Generate report content lines without borders
        lines = []

        # Title line
        title = f"RANGE MAKER BACKTEST RESULTS - {self.pair}"
        lines.append(title)
        lines.append("")
        ...
        lines.append(
            f"  Period: {self.config.period} | Timeframe: {self.config.timeframe} | Mode: {self.config.mode.value}")
        lines.append(f"  Trades Executed: {self.metrics.num_trades}")
        lines.append("")

        # Balance comparison
        lines.extend(self._generate_balance_comparison(initial_balances, final_balances))
        lines.append("")

        # Portfolio performance
        lines.append("PORTFOLIO PERFORMANCE (DUAL VIEW)")
        lines.append(f"  Base Token ({self.base_token}):")
        lines.append(f"    Initial Value: {self.metrics.initial_portfolio_value_asset1:.6f}")
        lines.append(f"    Final Value:   {self.metrics.final_portfolio_value_asset1:.6f}")
        lines.append(f"    Total Return:  {self.metrics.total_return_pct_asset1:.2f}%")
        lines.append(f"  Quote Token ({self.quote_token}):")
        lines.append(f"    Initial Value: {self.metrics.initial_portfolio_value_asset2:.6f}")
        lines.append(f"    Final Value:   {self.metrics.final_portfolio_value_asset2:.6f}")
        lines.append(f"    Total Return:  {self.metrics.total_return_pct_asset2:.2f}%")
        lines.append("")

        # Total fills
        lines.append(f"Total Fills: {self.metrics.total_fills}")
        lines.append("")

        # Price movement
        lines.append("PRICE MOVEMENT")
        lines.append(f"  Initial Price: {self.metrics.initial_price:.6f} {self.quote_token}/{self.base_token}")
        lines.append(f"  Final Price:   {self.metrics.final_price:.6f} {self.quote_token}/{self.base_token}")
        lines.append(f"  Price Return:  {self.metrics.price_return_pct:.2f}%")
        lines.append("")

        # Risk metrics
        lines.append("RISK METRICS")
        lines.append(f"  Max Drawdown:  {self.metrics.max_drawdown:.2f}% (Largest peak-to-trough decline)")
        lines.append(f"  Volatility:    {self.metrics.volatility:.2f}% (Annualized price fluctuation)")
        lines.append(f"  Sharpe Ratio:  {self.metrics.sharpe_ratio:.4f} (Risk-adjusted return)")
        lines.append("")

        # Impermanent loss
        lines.append("IMPERMANENT LOSS ANALYSIS (DUAL VIEW)")
        lines.append(
            f"  Final IL:      {final_il_base:.6f} {self.base_token} / {self.metrics.final_impermanent_loss:.6f} {self.quote_token}")
        lines.append(
            f"  Max IL:        {max_il_base:.6f} {self.base_token} / {self.metrics.max_impermanent_loss:.6f} {self.quote_token}")
        lines.append(
            f"  Average IL:    {avg_il_base:.6f} {self.base_token} / {self.metrics.avg_impermanent_loss:.6f} {self.quote_token}")
        lines.append("  IL Explained:  Loss from providing liquidity vs holding assets")
        lines.append("")

        # Strategy vs buy & hold
        lines.append("STRATEGY VS BUY & HOLD (DUAL VIEW)")
        lines.append(f"  Buy & Hold Return: {self.metrics.buy_hold_return_pct:.2f}% (Quote token perspective)")
        lines.append(f"  Strategy Return:   {self.metrics.total_return_pct:.2f}% (Quote token perspective)")
        lines.append(f"  Outperformance:    {self.metrics.outperformance_pct:.2f}% (Quote token perspective)")
        lines.append(
            f"  Base Token Outperformance: {self.metrics.total_return_pct_asset1 - self.metrics.price_return_pct:.2f}% (vs price movement)")
        lines.append("")

        # Metrics explanation
        lines.append("METRICS EXPLANATION:")
        lines.append("  • All metrics shown from both token perspectives (Base and Quote)")
        lines.append("  • Portfolio Value: Value in terms of each token")
        lines.append("  • Impermanent Loss: Difference vs holding, shown in both tokens")
        lines.append("  • Outperformance: Strategy vs buy-and-hold in both perspectives")

        # Find maximum line length
        max_len = max(len(line) for line in lines)

        # Build the bordered report
        bordered_lines = []
        ...
        for line in lines:
            if line == "":
                # Empty line becomes borderless separator
                bordered_lines.append("║" + " " * (max_len + 2) + "║")
            else:
                bordered_lines.append("║ " + line.ljust(max_len) + " ║")
    ...
    def _generate_balance_comparison(self, initial_balances: Dict[str, float],
                                     final_balances: Dict[str, float]) -> List[str]:
        """Generate balance comparison section as list of lines."""
        lines = []
        lines.append("BALANCE COMPARISON")
        ...
        lines.append(
            f"  {self.base_token}: {initial_base:.6f} → {final_base:.6f} ({base_change:+.6f}, {base_change_pct:+.2f}%)")
        ...
        lines.append(
            f"  {self.quote_token}: {initial_quote:.6f} → {final_quote:.6f} ({quote_change:+.6f}, {quote_change_pct:+.2f}%)")
...
class AnimationGenerator:
    ...
    def __init__(self, portfolio: PortfolioTracker, strategy_data: List[Dict]):
        ...
        self.logger = logging.getLogger("animation_generator")
        self.logger.setLevel(logging.DEBUG)

    def create_animation(self, save_path: str):
        """Create animated order book visualization with debug logging."""
        if not self.strategy_data:
            self.logger.warning("No animation data available")
            return

        # Log animation parameters
        self.logger.debug(f"Creating animation with {len(self.strategy_data)} frames")
        self.logger.debug(f"Base token: {self.base_token}, Quote token: {self.quote_token}")
        self.logger.debug(f"Saving to: {save_path}")
        ...
        try:
            ...
            self.logger.error(f"Animation save failed: {e}")
            self.logger.error("Please install ffmpeg: sudo apt install ffmpeg")
    ...
    def _update_frame(self, ax1, ax2, frame_data):
        """Update frame with current data and log values."""
        ax1.clear()
        ax2.clear()
        ...
        # Log frame details
        self.logger.debug(f"\n{'='*50}")
        self.logger.debug(f"Processing frame: {frame_data['timestamp']}")
        self.logger.debug(f"Current price: {frame_data['price']:.6f}")
        self.logger.debug(f"Current balances: {self.base_token}={frame_data['balances'].get(self.base_token, 0):.6f}, "
                         f"{self.quote_token}={frame_data['balances'].get(self.quote_token, 0):.6f}")
        ...
        # Plot buy orders with debug logging
        buy_prices = []
        buy_base_sizes = []
        buy_quote_sizes = []
        for order in frame_data['buy_orders'].values():
            ...
            self.logger.debug(f"BUY ORDER: Price={order['price']:.6f}, "
                             f"Base (taker)={order['taker_size']:.6f} {self.base_token}, "
                             f"Quote (maker)={order['maker_size']:.6f} {self.quote_token}")

        # Plot sell orders with debug logging
        sell_prices = []
        sell_base_sizes = []
        sell_quote_sizes = []
        for order in frame_data['sell_orders'].values():
            ...
            self.logger.debug(f"SELL ORDER: Price={order['price']:.6f}, "
                             f"Base (maker)={order['maker_size']:.6f} {self.base_token}, "
                             f"Quote (taker)={order['taker_size']:.6f} {self.quote_token}")

        # Log order summary
        if buy_prices:
            self.logger.debug(f"Buy orders count: {len(buy_prices)}, Min price: {min(buy_prices):.6f}, "
                             f"Max price: {max(buy_prices):.6f}")
        if sell_prices:
            self.logger.debug(f"Sell orders count: {len(sell_prices)}, Min price: {min(sell_prices):.6f}, "
                             f"Max price: {max(sell_prices):.6f}")
        ...
        ax1.set_title(f"Range Maker Strategy - {self.pair} at {frame_data['timestamp'].strftime('%Y-%m-%d')}",
                      fontsize=14)
...
class RangeMakerBacktester:
    ...
    async def run_backtest(self, pair_cfg: Dict[str, Any], initial_balances: Dict[str, float]) -> BacktestMetrics:
        ...
        try:
            ...
            # Initialize strategy with pair configuration
            self.strategy.initialize_strategy_specifics(**pair_cfg)

            # Set initial mid price from historical data
            self.strategy.update_mid_price(pair, initial_mid_price)
            ...
            # Set initial mid price in strategy
            self.strategy.update_mid_price(pair, initial_mid_price)
            ...
            if grid:
                metrics.total_fills = len(grid.filled_orders_history)
    ...
    def _setup_strategy_environment(self, pair: str, initial_balances: Dict[str, float]):
        ...
        self.logger.info(
            f"Mock DEX balances set: {base_token}={token1.dex.free_balance}, {quote_token}={token2.dex.free_balance}")
    ...
    async def _run_simulation(self):
        ...
        self.logger.debug(
            f"Initial orders generated. Active orders: {len(self.strategy.order_grids[pair].active_orders)}")
        ...
        if self.config.animate:
            ...
            self.logger.info(
                f"Buy orders: {len(initial_frame_data['buy_orders'])}, Sell orders: {len(initial_frame_data['sell_orders'])}")
        ...
        for idx, row in self.historical_data.iterrows():
            ...
            for order_idx, order in enumerate(filled_orders):
                # Execute the trade and get TradeRecord with base_price added
                trade = self.portfolio.execute_trade(order, timestamp, base_price=order.get('base_price'))
                if trade:
                    self.logger.info(
                        f"Trade #{order_idx + 1}: {trade.side.upper()} {trade.base_amount:.6f} @ {trade.price:.6f} (Base: {trade.base_price:.6f})")

                    # Record filled order in history
                    filled_record = {
                        **order,
                        'filled_at': timestamp
                    }
                    # Get the grid from the strategy's order grids
                    if pair in self.strategy.order_grids:
                        grid = self.strategy.order_grids[pair]
                        grid.filled_orders_history.append(filled_record)

                    self.logger.info(
                        f"Trade executed: {order['type']} {order['taker_size']:.6f} @ {order['price']:.6f}")
                ...
                # Update strategy's mid price to this fill price
                old_price = config.current_mid_price
                self.strategy.update_mid_price(pair, order['price'])
                self.logger.info(f"Updated mid price to fill price: {old_price:.6f} -> {order['price']:.6f}")
    ...
    def _generate_and_log_report(self, metrics: BacktestMetrics):
        ...
        if metrics.executed_orders:
            # Build order lines without borders
            lines = []
            title = "EXECUTED ORDERS (CHRONOLOGICAL)"
            lines.append(title)
            lines.append("")  # Empty separator line

            for i, trade in enumerate(metrics.executed_orders):
                line = (f"{i + 1:3d}. {trade.timestamp.strftime('%Y-%m-%d %H:%M:%S')} | {trade.side.upper():4} | "
                        f"{trade.base_amount:>10.6f} {self.portfolio.base_token} @ {trade.price:>10.6f} {self.portfolio.quote_token}/{self.portfolio.base_token} | "
                        f"Value: {trade.quote_amount:>10.6f} {self.portfolio.quote_token}")
                lines.append(line)

            # Find maximum line length
            max_len = max(len(line) for line in lines)

            # Build bordered order log
            bordered_lines = []
            top_border = "╔" + "═" * (max_len + 2) + "╗"
            bottom_border = "╚" + "═" * (max_len + 2) + "╝"
            bordered_lines.append(top_border)
            ...
            # Convert to single string and log
            order_log = "\n".join(bordered_lines)
            self.logger.info("\n" + order_log)
        else:
...
async def main():
    ...
    strategy.is_backtesting = True  # Flag for backtest mode
