...
class ValidatePercentArg(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        if not 0.001 <= values < 1:
            logging.error(
                "Invalid argument: %s must be between 0.001 (inclusive) and 1 (exclusive).",
                self.dest,
            )
            raise argparse.ArgumentError(
                self, "Value must be between 0.001 (inclusive) and 1 (exclusive)."
            )
...
def start():
    ...
    parser.add_argument(
        ...
        "--TokenToSell",
        required=True,
        help="Token to sell (e.g., BLOCK). Required.",
    )
    parser.add_argument(
        "-ttb",
        "--TokenToBuy",
        required=True,
        help="Token to buy (e.g., LTC). Required.",
    )
    parser.add_argument(
        "-atts",
        "--AmountTokenToSell",
