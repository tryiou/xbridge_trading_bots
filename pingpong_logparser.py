...
logger = setup_logging(name="GENERAL_LOG", level=logging.INFO, console=True)
    ...
    with open(log_file_path) as file:
        for line in file:
            ...
            if not stripped_line:
                continue
            dict_in_line = extract_dict_from_line(stripped_line)
            if "order FINISHED:" in stripped_line:
                name = dict_in_line["name"]
                finished_orders[name].append(dict_in_line)
            elif "virtual order:" in stripped_line:
                pass  # we don't need this line
            elif "xbridge order:" in stripped_line:
                id = dict_in_line["id"]
                xbridge_orders[id].append(dict_in_line)
            else:
                logger.warning(f"Failed to identify log line ? {stripped_line}")
...
def _get_xbridge_order_details(order_id, instance_name, xbridge_orders):
    ...
    if not xbridge_order_list:
        logger.warning(
            f"Could not find xbridge order details for finished order ID: {order_id}"
        )
        return None
...
def process_orders(finished_orders, xbridge_orders):
    ...
    for instance_name, orders_list in finished_orders.items():
        ...
        for order in orders_list:
            if order["side"] == "SELL":
                current_sell = _get_xbridge_order_details(
                    order["orderid"], instance_name, xbridge_orders
                )
                if not current_sell:
                    continue
            elif order["side"] == "BUY" and current_sell is not None:
                current_buy = _get_xbridge_order_details(
                    order["orderid"], instance_name, xbridge_orders
                )
                if not current_buy:
                    continue
    ...
    for sell_order, buy_order in completed_cycles:
        ...
        try:
            sell_time = datetime.strptime(
                sell_order.get("updated_at", "1970-01-01T00:00:00Z"),
                "%Y-%m-%dT%H:%M:%S.%fZ",
            )
            buy_time = datetime.strptime(
                buy_order.get("created_at", "1970-01-01T00:00:00Z"),
                "%Y-%m-%dT%H:%M:%S.%fZ",
            )
...
def display_tables(completed_data, inprogress_data, profit_info):
    ...
    if completed_data:
        ...
        print(
            tabulate(
                completed_data, headers=headers, tablefmt="pretty", colalign=colalign
            )
        )
    else:
    ...
    for instance_name, data in profit_info.items():
        ...
        asset = data["asset"]
        profit_table_data.append((instance_name, f"{total_profit:.6f}", asset or ""))
    ...
    if profit_table_data:
        ...
        print(
            tabulate(
                profit_table_data,
                headers=["Instance", "Total Profit", "Asset"],
                tablefmt="pretty",
...
def main():
    ...
    completed_cycles, in_progress_cycle = process_orders(
        finished_orders, xbridge_orders
    )
