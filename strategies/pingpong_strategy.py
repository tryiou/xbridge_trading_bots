...
class PingPongStrategy(MakerStrategy):
    ...
    def initialize_strategy_specifics(self, **kwargs):
        ...
        enabled_pairs = [
            p for p in self.config_pingpong.pair_configs if p.get("enabled", True)
        ]
        if not enabled_pairs:
            self.config_manager.general_log.info(
                "  - No enabled pairs found in config_pingpong.yaml."
            )
        else:
            self.config_manager.general_log.info(
            ...
            for pair_cfg in enabled_pairs:
                self.config_manager.general_log.info(
                    ...
                    pair_cfg["name"],
                    pair_cfg["pair"],
                    pair_cfg.get("usd_amount", 0.0),
                    pair_cfg.get("spread", 0.0) * 100,
                    pair_cfg.get("sell_price_offset", 0.0) * 100,
                )
        self.config_manager.general_log.info("------------------------------------")
    ...
    def get_pairs_for_initialization(self, tokens_dict, **kwargs) -> dict:
        return self._create_pairs_from_configs(
            self.config_pingpong.pair_configs, tokens_dict, "pingpong", **kwargs
        )
