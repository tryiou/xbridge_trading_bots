...
class BasicSellerStrategy(MakerStrategy):
    ...
    def initialize_strategy_specifics(self, token_to_sell=None, token_to_buy=None, amount_token_to_sell=None,
        ...
        self.config_manager.general_log.info("  - Token to Sell: %s", self.token_to_sell)
        self.config_manager.general_log.info("  - Token to Buy: %s", self.token_to_buy)
        
        if self.amount_token_to_sell is not None:
            self.config_manager.general_log.info("  - Amount to Sell: %s %s", self.amount_token_to_sell, self.token_to_sell)
        else:
            self.config_manager.general_log.info("  - Amount to Sell: None")
            
        if self.min_sell_price_usd is not None:
            self.config_manager.general_log.info("  - Minimum Sell Price: $%.4f USD", self.min_sell_price_usd)
        else:
            self.config_manager.general_log.info("  - Minimum Sell Price: None")
            
        if self.sell_price_offset is not None:
            self.config_manager.general_log.info("  - Sell Price Upscale: %.2f%%", self.sell_price_offset * 100)
        else:
            self.config_manager.general_log.info("  - Sell Price Upscale: None")
            
        if self.partial_percent:
            self.config_manager.general_log.info("  - Partial Order Minimum Size: %.1f%% of total", self.partial_percent * 100)
    ...
    def get_pairs_for_initialization(self, tokens_dict, **kwargs) -> dict:
        ...
        return self._create_pairs_from_configs(self.config_manager.config_basicseller.seller_configs, tokens_dict, "basic_seller", **kwargs)
    ...
    def build_buy_order_details(self, dex_pair, manual_dex_price=None) -> tuple:
        self.config_manager.general_log.error("Bot strategy is basic_seller, no rule for this strat on build_buy_order_details")
        return 0, 0

    def determine_buy_price(self, dex_pair, manual_dex_price=None) -> float:
        self.config_manager.general_log.error("Bot strategy is basic_seller, no rule for this strat on determine_buy_price")
        return 0
    ...
    async def handle_finished_order(self, dex_pair, disabled_coins: list):
        self.config_manager.general_log.info("Sell order for '%s' completed successfully. Disabling instance.", dex_pair.pair.name)
