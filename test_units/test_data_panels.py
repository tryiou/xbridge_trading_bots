...
from gui.components.data_panels import BalancesPanel, OrdersPanel
...
def test_column_sorting_logic():
    ...
    mock_tree_manager = TreeManager(
        mock_frame, columns=[("col1", "Col1", 10)], redraw_callback=lambda: None
    )
    ...
    for value, expected_sort_key in test_cases:
        ...
        if isinstance(expected_sort_key[1], float) and math.isnan(expected_sort_key[1]):
            assert math.isnan(actual_sort_key[1]), (
                f"For value '{value}', expected NaN but got {actual_sort_key[1]}"
            )
            assert actual_sort_key[0] == expected_sort_key[0], (
                f"For value '{value}', expected type {expected_sort_key[0]} but got {actual_sort_key[0]}"
            )
        else:
            assert actual_sort_key == expected_sort_key, (
                f"For value '{value}', expected {expected_sort_key} but got {actual_sort_key}"
            )
    ...
    sorted_data_desc = sorted(
        mixed_data, key=lambda k: get_sort_value(k.get("col", "")), reverse=True
    )
    actual_sort_keys_desc = [
        get_sort_value(item.get("col")) for item in sorted_data_desc
    ]
...
def test_balances_panel_total_usd_calculation(tk_root_for_panels):
    ...
    sample_data = [
        ...
        {"symbol": "LTC", "usd_price": 150.0, "total": 10, "free": 8},
        {
            ...
            "total": 20,
            "free": 20,
        },  # Added usd_price to avoid KeyError
    ]
    ...
    btc_data = next(
        (item for item in panel.tree_manager.current_data if item["symbol"] == "BTC"),
        None,
    )
    ltc_data = next(
        (item for item in panel.tree_manager.current_data if item["symbol"] == "LTC"),
        None,
    )
    nocex_data = next(
        (item for item in panel.tree_manager.current_data if item["symbol"] == "NOCEX"),
        None,
    )
