...
class GUITester:
    ...
    def run_all_tests(self):
        ...
        test_methods = [
            ...
            self._test_column_sorting_logic # New test case
        ]
    ...
    def _test_column_sorting_logic(self):
        """Test the _get_sort_value method for robust column sorting."""
        test_name = "Column Sorting Logic"
        print(f"\n--- [TEST CASE] Running: {test_name} ---")
        passed = False
        try:
            ...
            test_cases = [
                ...
                ("BTC/USD", (1, "btc/usd")),
                ("PENDING", (1, "pending")),
                # Mixed type sorting check (conceptual, actual sort happens in sorted() call)
                # This test only checks the output of _get_sort_value for individual items
            ]
            ...
            for value, expected_sort_key in test_cases:
                ...
                if isinstance(expected_sort_key[1], float) and math.isnan(expected_sort_key[1]):
                    ...
                    assert actual_sort_key[0] == expected_sort_key[0], f"For value '{value}', expected type {expected_sort_key[0]} but got {actual_sort_key[0]}"
                else:
                    assert actual_sort_key == expected_sort_key, f"For value '{value}', expected {expected_sort_key} but got {actual_sort_key}"
                print(f"  - Value: '{value}' -> Sorted as: {actual_sort_key} (Expected: {expected_sort_key})")
            ...
            # Handle NaN comparison separately if needed, but for this set, no NaNs are expected in the final sorted list.
            ...
            print("[TEST PASSED] Column sorting logic handles various data types correctly with comprehensive mixed data.")
            ...
            print("[TEST PASSED] Column sorting logic handles descending order correctly.")
            passed = True
        except Exception as e:
            print(f"[TEST FAILED] {e}")
            traceback.print_exc()
