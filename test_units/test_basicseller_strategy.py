...
class BasicSellerStrategyTester:
    ...
    async def _test_sell_price_above_minimum(self):
        ...
        with self._patch_dependencies():
            ...
            self._set_mock_prices(
                t1_usd_price=1.0, t2_usd_price=0.1
            )  # Live price is $1.0
            ...
            # Assert
            assert sell_price == cex_price, (
                "Should use CEX price when it's higher than the minimum"
            )
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly used CEX price."
            )
    ...
    async def _test_sell_price_below_minimum(self):
        ...
        with self._patch_dependencies():
            ...
            assert sell_price == expected_price, (
                "Should use minimum USD price converted to pair price"
            )
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly used minimum price."
            )
    ...
    async def _test_order_completion_disables_pair(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            assert self.pair.dex.disabled is True, (
                "Pair was not disabled after order finished."
            )
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly disabled pair on order completion."
            )
    ...
    async def _test_price_variation_recreates_order(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly recreated order on price variation."
            )
    ...
    async def _test_error_swap_status_disables_pair(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly disabled pair on error status."
            )
    ...
    def _test_strategy_static_values(self):
        ...
        self.config_manager.general_log.info(
            "[TEST PASSED] Static value methods returned correct values."
        )
