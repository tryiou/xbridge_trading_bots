...
class PingPongStrategyTester:
    ...
    def __init__(self, strategy_instance: "PingPongStrategy"):
        ...
        if not self.config_manager.pairs:
            raise RuntimeError(
                "Cannot run tests: No pairs were initialized. "
    ...
    def _patch_dependencies(self):
        ...
        with (
            patch.object(
                self.config_manager.xbridge_manager, "makeorder", new_callable=AsyncMock
            ) as mock_make_order,
            patch.object(
    ...
    async def _test_initial_sell_order_creation(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly created a SELL order."
            )
    ...
    async def _test_buy_order_creation_after_sell(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly created a BUY order."
            )
    ...
    async def _test_order_expiration_recreates_order(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly recreated a SELL order after expiration."
            )
    ...
    async def _test_buy_price_logic_on_market_moves(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.config_manager.general_log.info(
                "  - Testing favorable market move (price drops)..."
            )
    ...
    async def _test_insufficient_balance(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.pair.t1.dex.free_balance = (
                0.1  # Virtual order will need more than this
            )
    ...
    async def _test_error_swap_status_disables_pair(self):
        ...
        with self._patch_dependencies() as mocks:
            # Arrange: An open SELL order exists
            self.pair.dex.order = {"id": "mock_order_id_123", "status": "open"}
            assert self.pair.dex.disabled is False, (
                "Pair should not be disabled initially."
            )
            ...
            assert self.pair.dex.disabled is True, (
                "Pair was not disabled after 'error swap' status."
            )
            self.config_manager.general_log.info(
    ...
    async def _test_cancel_own_orders(self):
        ...
        with self._patch_dependencies() as mocks:
            ...
            self.config_manager.general_log.info(
                "[TEST PASSED] Correctly cancelled its own order."
            )
            ...
            mocks["cancel_order"].assert_not_called()
            self.config_manager.general_log.info(
                "[TEST PASSED] Did not attempt to cancel when no order exists."
            )
    ...
    async def _test_build_sell_order_details_calculation(self):
        ...
        with self._patch_dependencies():
            ...
            # Act
            amount, _offset = self.strategy.build_sell_order_details(self.pair.dex)
    ...
    async def _test_buy_order_price_lock(self):
        ...
        with self._patch_dependencies():
            ...
            _variation_high, is_locked_high = (
                self.strategy.calculate_variation_based_on_side(
