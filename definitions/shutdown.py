...
async def wait_for_pending_rpcs(
    ...
    while time.time() - start_time < timeout:
        ...
        if int(elapsed) % 5 == 0:
            logger.info(
                f"Waiting on {active_rpcs} RPCs ({int(elapsed)}s/{timeout}s)..."
            )

        await asyncio.sleep(1)

    logger.warning(
        f"RPC wait timeout after {timeout} seconds, proceeding with shutdown"
    )
...
class ShutdownCoordinator:
    ...
    @staticmethod
    async def unified_shutdown(config_manager: Optional["ConfigManager"]) -> None:
        """Core shutdown logic for both CLI and GUI per strategy"""
        from strategies.maker_strategy import MakerStrategy

        try:
            logger = (
                config_manager.general_log
                if config_manager
                else logging.getLogger("unified_shutdown")
            )
            ...
            if config_manager.strategy_instance:
                try:
                    if isinstance(config_manager.strategy_instance, MakerStrategy):
                        ...
                        count = (
                            await config_manager.strategy_instance.cancel_own_orders()
                        )
                        logger.info(f"Cancelled {count} strategy orders")
                    else:
                        logger.info(
                            "Skipping order cancellation - not a maker strategy"
                        )
            ...
            if config_manager:
                await config_manager.error_handler.handle_async(
                    e, context={"phase": "shutdown"}
                )
            else:
                # If config_manager is None, we have no error handler, so just log
                logging.getLogger("unified_shutdown").critical(
                    f"Unhandled exception during shutdown: {e}", exc_info=True
                )
