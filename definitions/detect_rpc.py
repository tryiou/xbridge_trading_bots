...
def load_config_path_from_json(json_path):
    if os.path.exists(json_path):
        logging.info(f'Loading config path from {json_path}')
        with open(json_path, 'r') as json_file:
            json_data = json.load(json_file)
            if 'blocknet_path' in json_data and os.path.exists(json_data['blocknet_path']):
                logging.info(f'Loaded config path: {json_data["blocknet_path"]}')
...
def get_default_config_path():
    ...
    if os.path.exists(default_path):
        logging.info(f'Using default config path: {default_path}')
        return default_path
    else:
        logging.warning(f'Default config path does not exist: {default_path}')
        return ''
...
def prompt_user_for_config_path():
    ...
    try:
        ...
        if config_path:
            logging.info(f'User selected config path: {config_path}')
        else:
            logging.warning('User canceled the file dialog')
    except ImportError:
        logging.error('Tkinter or ttkbootstrap is not available. Asking for config path in terminal.')
        time.sleep(0.1)
...
def save_config_path_to_json(json_path, config_path):
    ...
    logging.info(f'Saved config path to {json_path}: {config_path}')
