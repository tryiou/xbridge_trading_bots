...
formatter = logging.Formatter(
    "[%(asctime)s] [%(correlation_id)s] [%(name)-20s] %(levelname)-8s - %(message)s"
)
...
def setup_logging(
    ...
    if console and not _BACKTEST_MODE_ACTIVE:
        ...
        ch.setFormatter(
            ColoredFormatter(
                "[%(asctime)s] [%(correlation_id)s] [%(name)-20s] %(levelname)s - %(message)s"
            )
        )
        ch.setLevel(level)
...
def setup_logger(
    ...
    general_log = setup_logging(
        name=f"{strategy}.general",
    ...
    trade_log = setup_logging(
        name=f"{strategy}.trade",
        log_file=os.path.join(logs_dir, strategy + "_trade.log"),
        level=logging.INFO,
        console=False,
    )

    ccxt_log = setup_logging(
        name=f"{strategy}.ccxt",
